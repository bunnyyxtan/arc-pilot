import { DISPUTE_STATUS_COLORS, type DisputeStatusLabel } from "../../lib/disputes/status";

export function DisputeStatusBadge({ status, className = "" }: { status: DisputeStatusLabel; className?: string }) {
  const colorClasses = DISPUTE_STATUS_COLORS[status] || DISPUTE_STATUS_COLORS["Awaiting resolution"];
